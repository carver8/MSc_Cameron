...
def compute_interp(lat,lon,field):
  ...
  ## Pyresample -- one particular tool
  # reference grid os obs grid (NSIDC data grid) (reference grid)
  # Interpolate model grid (native grid) onto reference grid
  if np.min(lat) > 0:
    # Northern Hemisphere
    # load lat-lon of the target grid
    access_pr_file = '/sea ice data/OBS/siconc/NSIDC-0051/siconc_r1i1p1_mon_197901-201712_nh-psn25.nc' ### Replace with ancillary file
...
#### Load lat and lon from anciillary file
# Load files from NSIDC model 
# Load sicon with xarray
# Put thoaw variables into interpolation
