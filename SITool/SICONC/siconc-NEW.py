...
obs_ds = obs_ds.sel(tdim=slice('2008-01-01','2009-12-31')) #Time slice of data
...
access_model_data = 'MODELS/SICONC/siconc_SImon_CMCC-CM2-SR5_omip2_r1i1p1f1_gn_165301-201812_reproj.nc' #access reprojected model sea ice dataset
model_ds = xr.open_dataset(access_model_data) # Open dataset
# model_ds = model_ds.siconc
# model_ds = model_ds.sel(time=slice('1979-01-01','2009-12-31')) #Time slice of data
model_ds = model_ds.sel(time=slice('2008-01-01','2009-12-31')) #Time slice of data
model_da = model_ds.siconc
mv = model_da[1,:,:].values
# %%% Monthly Mean Cycle
...
conc1 = model_mean_mon   
    ...
    # Observational
obs_mean_mon = obs_ds.cdr_seaice_conc_monthly.groupby('time.month').mean('tdim')
conc2 = obs_mean_mon
...
ano_conc2 = obs_ds.cdr_seaice_conc_monthly.groupby('tdim') - monthly_mean_aligned
...
trend_ano_conc2 = obs_ds.cdr_seaice_conc_monthly.polyfit(dim='tdim', deg=1)
...
for i in range(0,12,1):
    ...
    plt.title(f'Mod v Obs Difference for {months[i]}')
